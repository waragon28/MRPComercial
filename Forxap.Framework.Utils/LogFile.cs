...
{
    /// <summary>
    ///  Clase para administrar un Log de ocurrencias
    /// </summary>
    /// <license>
    
    /// 
    /// Copyright (c) 2013 - 2014 Percy Lazarte
    /// 
    /// </license>
    /// <remarks>
    ...
    public static class LogFile
    {
        ...
        public static Exception SetLogDir(string logDir, bool createIfNotExisting = false)
        {
            ...
            try
            {
                ...
                if (!_logDir.Exists)
                {
                    ...
                    {
                        throw new DirectoryNotFoundException(string.Format("Directorio '{0}'  no existe!", _logDir.FullName));
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <param name="useBackgroundTask">Whether to use the background task (thread) to write messages to disk. Default is true. This is much faster than writing directly to disk in the main thread.</param>
        /// <param name="framesToSkip">How many frames to skip when detecting the calling method, <see cref="GetCaller"/>. This is useful when log calls to <see cref="LogFile"/> are wrapped in an application. Default is 0.</param>
        /// <returns>Null on success or the <see cref="Exception"/> that occurred when processing the message, i.e. when enqueuing the message (when <paramref name="useBackgroundTask"/> is true) or when writing the message to disk (when <paramref name="useBackgroundTask"/> is false).</returns>
        ...
        private static Exception WriteLogEntryToFile(XElement xmlEntry)
        {
            ...
            try
            {
                return new Exception(string.Format("No se puede escribir en el archivo '{0}', porque esta bloqueado por otro proceso, por mas de {1} segundos.", FileName, secondsToWaitForFile));
            }
            catch (Exception ex)
            {
                return ex;
        ...
        /// </summary>
        /// <remarks>
        /// The method is walking up the frames in the stack trace until the first method outside <see cref="LogFile"/> is reached.
        /// When log calls to <see cref="LogFile"/> are wrapped in an application, this may still not be the method where logging 
        ...
        /// </remarks>
        /// <param name="framesToSkip">How many frames to skip when detecting the calling method. This is useful when log calls to <see cref="LogFile"/> are wrapped in an application. Default is 0.</param>
        ...
        private static string GetCaller(int framesToSkip = 0)
        {
            ...
            while (true)
            {
                ...
                if (declaringType != typeof(LogFile) && --framesToSkip < 0)
                    break;
            }
